...
export function tagVersionCli(): Command<any, any, any, any, any, any, any, any> {
  return new Command()
    .name("tag-version")
    .description("Cria e publica uma tag git baseada na versão do deno.json[c] (vMAJOR.MINOR)")
    .version(APP_VERSION)
    .option("-m, --message <msg:string>", "Mensagem personalizada do commit")
    .option("-s, --sanitize", "Sanitiza o arquivo deno.json[c] em disco antes do commit", {
      default: false,
    })
    ...
    .option("--dry-run", "Simula as operações git sem efetuar commits ou pushes", {
      default: false,
    })
